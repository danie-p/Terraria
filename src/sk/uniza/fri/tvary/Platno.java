...
public class Platno {
    ...
    public static final int SIRKA_PLATNA = (dimension.width / Zem.MAX_SIRKA_ZEME) * Zem.MAX_SIRKA_ZEME / 10 * 9; // sirka platna sa odvija od sirky obrazovky (tvori cca 90%), nasledne sa vypocita velkost policka
    public static final int VYSKA_PLATNA = Zem.MAX_VYSKA_ZEME * (Platno.SIRKA_PLATNA / Zem.MAX_SIRKA_ZEME); // vyska platna sa vypocita z velkosti policka
    // velkost platna, a teda aj velkost policka zavisi na velkosti obrazovky
    // pocet policok zostava rovnaky pre vsetky obrazovky, meni sa len velkost policka
    ...
    public static Platno dajPlatno() {
        if (Platno.platnoSingleton == null) {
            Platno.platnoSingleton = new Platno("Terraria", Platno.SIRKA_PLATNA, Platno.VYSKA_PLATNA, Color.cyan);
        }
    ...
    private Platno(String titulok, int sirka, int vyska, Color pozadie) {
        ...
        this.frame.setLocation((dimension.width - sirka) / 2, (dimension.height - vyska) / 2 - 30);
        ...
        JMenuItem ukonciMenu = new JMenuItem("Ukonči");
        menuBar.add(ukonciMenu);
        ukonciMenu.addActionListener(new ActionListener() {
            @Override
        ...
        this.canvas.setPreferredSize(new Dimension(Platno.SIRKA_PLATNA, Platno.VYSKA_PLATNA + menuBar.getHeight()));
    ...
    private void akciaUkoncenie() {
        if (JOptionPane.showConfirmDialog(this.frame,  "Chceš naozaj ukončiť program?",
                "Ukončenie", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.YES_OPTION ) {
            System.exit(0);
        }
        this.frame.repaint();
    }
